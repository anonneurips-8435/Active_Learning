...
class GLISTER(Strategy):
    ...
    def __init__(self,X, Y,unlabeled_x, net, handler, nclasses, args,valid,X_val=None,Y_val=None,\
        ...
        self.gradient_comb = gradient_comb
    ...
    def _update_grads_val(self,grads_currX=None, first_init=False):
        ...
        if first_init:
            ...
            with torch.no_grad():
                ...
                for x, y, idxs in loader:
                    ...
                    if first:
                       self.grads_val_curr = torch.cat((l0_grads, l1_grads), dim=1) 
                       first = False
                    else:
                       self.grads_val_curr = torch.cat((self.grads_val_curr,\
        ...
        curr_len = len(self.grads_val_curr)
        labels = torch.arange(int(curr_len/self.gradient_comb)+1).\
            repeat_interleave(self.gradient_comb).to(self.device)
        
        M = torch.zeros(labels.max()+1, curr_len,device=self.device)
        M[labels[:curr_len], torch.arange(curr_len)] = 1
        M = torch.nn.functional.normalize(M, p=1, dim=1)
        self.grads_val_curr = torch.mm(M, self.grads_val_curr)
    
    def eval_taylor_modular(self, grads,greedySet=None,remset=None):

        with torch.no_grad():
            gains, _ = torch.matmul(grads, self.grads_val_curr.T).max(dim=0) #.mean(dim=0)
            gains = - gains
            if self.typeOf == "FacLoc":
                gains = gains + self.lam*((self.min_dist - \
                    torch.min(self.min_dist,self.sim_mat[remset])).sum(1)).to(self.device)
                
            elif self.typeOf == "Diversity" and len(greedySet) > 0:
                gains = gains - \
                    self.lam*self.sim_mat[remset][:, greedySet].sum(1).to(self.device)
            #else:
